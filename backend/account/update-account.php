...
try {
    if (isset($_POST["update_account"])) {
        ...
        if ($submitted_email !== $email) {
            $update_email = $pdo->prepare("UPDATE users SET email = :email_new WHERE email=:email");
            $update_email->bindParam(':email_new', $user_email);
        ...
        if (!empty($old_password) && !empty($new_password)) {
            ...
            if (password_verify($old_password, $account_info[0]['password']) && !empty($new_password)) {
                ...
                if ($update_result) {
                    $_SESSION['Password_Changed'] = "Password has been updated.";
                    header("location: ../../account-settings");
                    exit;
                }
            } else {
